...
// 한번만 사용하는 것이 좋습니다.
// document.ready() : HTML 문서를 구조적으로 표현하는 DOM이 모두 로딩되어 준비된 후 자바스크립트 문서 코드가 실행되도록 함
...
$(document).ready(function(){
    ...
    // slideDown(), slideUp() : jQuery 라이브러리에서 제공하는 함수로 슬라이딩 애니메이션과 함께 보여주거나 숨김 구현함. 선택한 요소의 height값을 낮추거나 높여가며 슬라이드 업 또는 다운 전환되게 함. 이때, 매개변수값(인수) 500은 0.5초를 의미함
      ...
      // .imgslide a:gt(0) : gt(index)은 index값보다 더 큰 값(Greater)을 가진 요소를 선택함. 0번째부터 계수하여 index 값보다 큰 값을 가져옴. 여기서는 gt(0)이므로, 1, 2, 3이 됨. 즉, 0보다 큰 인덱스는 hide() 메서드를 적용함
      $('.imgslide a:gt(0)').hide();
    
      // setInterval(function(){}), 3000 : 일정시간마다 반복적으로 동작을 실행하게 함.
      // 3000은 3000ms(3초)는 3초마다 반복 실행함.
      setInterval(function(){
          // 가상클래스 선택자로 부모 요소가 가지고 있는 자식 요소 중 첫번째를 선택함. imgslide의 자식 요소 a태그에서 첫 번째를 선택하여 페이드 아웃 실행함
          ...
          // $(".notice li :first").click의미는 ".notice 요소의 후손 요소 중에서" 첫번째 요소를 클릭하면" 입니다."

    $(function(){
        //('.tabmenu>li>a').click: .tabmenu요소의 자식요소 <li>의 자식요소인 a 태그영역을 클릭하면
        $('.tabmenu>li>a').click(function(){
            //(this).paraent().addClass("active"):현재 태그의 부모태그를 찾아 'active'클래스를 추가해줌, a태그의 부모태그는 li이므로, 클릭하면 li에 'active'클래스를 추가해줍니다.
            $(this).parent().addClass("active")
            //.sibling().removeClass("active"):다른 형제 태그를 찾은 후 'active'클래스를 삭제해줍니다.
